...
SHAPES = Group(
    ...
    (
        ...
        Term(
            name="RTX",
            short="One node of RTL. A code, a machine mode, and some operands.",
            long="Every parenthesis you see in an RTL dump is one of these and they are all the same C struct, which is why the dumps look so uniform. The code says what kind of thing it is, `plus` or `reg` or `set`, and it is one of 203 codes listed in one file. There is no separate type for an instruction and an expression: an insn is an RTX whose code happens to be `insn`.",
            cite="gcc/rtl.h:319@releases/gcc-16.2.0",
            also=("`rtx`", "`rtx_def`"),
            see=("RTL", "machine mode", "insn"),
            met="T07",
        ),
        Term(
            name="machine mode",
            short="The size and kind of a value, written after the colon in a dump.",
            long="`SI` is four bytes of integer, `DI` is eight, `QI` is one, and the letters are historical rather than descriptive. Every RTX carries one, including ones where it means nothing, and the mode is what stops the back end from having to look at a type. `CC` and its variants are the odd ones: they are the mode of a condition code, and what a target keeps in one is up to the target.",
            cite="gcc/machmode.def:211@releases/gcc-16.2.0",
            also=("`SImode`", "`machine_mode`"),
            see=("RTX", "RTL"),
            met="T07",
        ),
        Term(
            name="pseudo register",
            short="A register the compiler invented, numbered from the first free number up.",
        ...
        Term(
            name="insn",
            short="One entry in the chain of things the function will do, in order.",
            long="After expand a function is a doubly linked list of these rather than a graph of blocks, though the blocks are still recorded on the side. Most entries are not instructions at all: notes, labels and debug entries share the chain, and in a small function they outnumber the real work. The spelling is missing a vowel because it is older than filenames that could hold one.",
            cite="gcc/rtl.def:145@releases/gcc-16.2.0",
            also=("`rtx_insn`",),
            see=("RTX", "RTL", "basic block"),
            met="T07",
        ),
    ),
)
